...
A1Kinematics::A1Kinematics()
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        rho_opt << 0.0, 0.0, 0.0;
        rho_fix_list.emplace_back(rho_fix);
        rho_opt_list.emplace_back(rho_opt);
    }
}
...
Eigen::Vector3d A1Kinematics::fk(Eigen::Vector3d q, int leg_index) {
    Eigen::Vector3d out;
    autoFunc_fk_derive(q.data(), rho_opt_list[leg_index].data(), rho_fix_list[leg_index].data(), out.data());
    return out;
}
...
Eigen::Matrix3d A1Kinematics::jac(Eigen::Vector3d q, int leg_index) {
    Eigen::Matrix3d mtx;
    autoFunc_d_fk_dq(q.data(), rho_opt_list[leg_index].data(), rho_fix_list[leg_index].data(), mtx.data());
    return mtx;
}

Eigen::Vector3d A1Kinematics::ik(Eigen::Vector3d foot_pos_r, int leg_index, Eigen::Vector3d initial_q, double eps, int max_iter)
{
    double p = 10;
    int iter_num = 0;
    Eigen::Matrix<double, 12, 1> joint_pos_q;
    joint_pos_q << 0.162668, 0.822337, -1.83411, -0.147463, 0.821221, -1.83381, 0.163412, 0.826852, -1.84372, -0.150993, 0.825153, -1.84135;
    Eigen::Vector3d q = joint_pos_q.segment<3>(3 * leg_index);
    Eigen::Vector3d err = foot_pos_r - fk(q, leg_index);  
    while(err.norm() > eps && iter_num <= max_iter)
    {
        Eigen::Vector3d dq = p * jac(q, leg_index).transpose() * err;
        q += dq;
        err = foot_pos_r - fk(q, leg_index);  
        // std::cout << iter_num << ": " << err.norm() <<std::endl;
        iter_num ++;
    }
    return q;
}
